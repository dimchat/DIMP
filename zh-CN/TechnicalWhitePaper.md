...
本协议首先定义了点对点的聊天（私聊）消息格式，然后在此基础上扩展了多对多的聊天（群聊）消息格式。

### <span id="personal-message">二人私聊</span>

当一个用户需要发送信息给另一个用户时，客户端需要先执行以下3步操作，再将计算结果发送到 DIM 网络中：
...
### <span id="group-message">多人对话</span>

在群聊天场景中，最简单的实现机制（我将其称之为 Polylogue）仅仅需要对上述的“二人私聊”逻辑做一下简单的升级即可：

1. 首先，信息打包的时候需要在信息内容 content 中增加 **group** 字段，填入对应的 Group ID，然后再开始进行打包发送操作；
2. 打包过程第一步（跟私聊的情况相同）由消息发送者生成一个随机的对称密码 PW，并用 PW 对待发送消息内容进行加密；
3. 打包过程第二步（跟私聊的情况略有不同）用**所有群成员**的公钥对这个密码 PW 分别进行加密，一并打包成 keys 字典附在信息包中；
4. 打包过程第三步（跟私聊的情况相同）用发送者的私钥对密文进行签名。

多人对话信息的接收跟私聊的情况类似：

1. 解包过程第一步（跟私聊的情况相同）先用发送者的公钥验证签名信息；
2. 解包过程第二步（跟私聊的情况略有不同）从 keys 中找到属于自己的密钥信息，然后再用自己的私钥解密得到 PW；
3. 解包过程第三步（跟私聊的情况相同）用密码 PW 对密文进行解密，得到信息内容 content；
4. 最后，检查信息内容中是否存在 group 字段，如果有，则判定为群消息并交由相应的群消息处理器进行处理。

### 扩展功能点

1. 为了节省流量，PW 应该允许复用，即在一定时间间隔内（比如24小时）发消息时无需再生成新的 PW（也无需再把该 PW 打包），客户端应保存每一个“**消息方向**”所对应的历史 PW。当接收到的新消息中找不到对应的 key 时，应该根据该“消息方向”从历史记录中寻找 PW 记录；
2. 考虑到可能丢失信息包（或者信息包顺序错乱）的情况，在升级功能点1的同时，需要支持查询 PW 的功能，以便接收方在无法找到有效的 PW 时，可以向消息发送方请求获取 PW，或者直接要求对方重发该信息（带上 key 字段）；
3. 由于接收方并不关心此信息包中的其他群成员的 key（每一个“消息方向”的 key 都不同），所以可以由 SP 提供**信息包裁剪服务**，即在向客户端推送消息时，如果消息包中有多余的 keys 信息，则仅提取出该接受者所需的 key，自动丢弃其余无用 keys 信息。
